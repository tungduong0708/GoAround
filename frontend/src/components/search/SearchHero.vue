...
<template>
  <section class="w-full px-4 py-8 sm:py-12">
    <Card class="mx-auto w-full max-w-5xl border border-border/60 bg-card/90 px-6 py-10 shadow-2xl backdrop-blur">
      <CardHeader class="items-center space-y-3 text-center">
        <CardTitle class="text-4xl font-semibold tracking-[0.35em] uppercase text-foreground sm:text-5xl">
          Go Around With Us
        </CardTitle>
      </CardHeader>

      <CardContent class="flex flex-col gap-6">
        <Tabs v-model="selectedCategory" class="w-full" :default-value="categories[0]?.value">
          <TabsList class="flex w-full justify-center gap-2 rounded-full border border-border/40 bg-secondary/30 p-7" aria-label="Search filters">
            <TabsTrigger
        ...
        <div class="flex w-full justify-center">
          <SearchBar
            ...
            :placeholder="props.placeholder ?? 'Search destinations, guides, or experiences'"
