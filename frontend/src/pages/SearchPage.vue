	...
	<div class="mx-auto w-full max-w-6xl px-4 py-10">
		<section class="space-y-6">
			<div class="flex flex-wrap items-end justify-between gap-4">
				<div>
					<p class="text-xs font-semibold uppercase tracking-[0.5em] text-muted-foreground">Search</p>
					<h1 class="text-4xl font-semibold tracking-tight text-foreground">Search Place</h1>
				</div>
				<div class="flex flex-wrap items-center gap-3">
					<Tabs v-model="selectedCategory" class="w-full sm:w-auto" :default-value="categories[0]?.value">
						<TabsList class="flex flex-wrap gap-2 rounded-full border border-border/60 bg-secondary/30 p-2">
							<TabsTrigger
			...
			<div class="flex flex-wrap items-center gap-4">
				<div class="w-full flex-1 min-w-[240px]">
					<SearchBar v-model="searchTerm" :loading="loading" placeholder="Search" @submit="performSearch" />
				</div>
			</div>
		</section>
		...
		<section class="mt-10 space-y-4">
			...
			<div v-if="error" class="rounded-xl border border-destructive/40 bg-destructive/5 p-4 text-destructive">
				<p>{{ error }}</p>
				<Button variant="destructive" class="mt-3" @click="performSearch">Try again</Button>
			</div>
			...
			<div v-else>
				...
				<div v-else-if="!hasResults" class="flex flex-col items-center gap-3 rounded-3xl border border-dashed border-border/70 p-10 text-center text-muted-foreground">
					<p class="text-lg font-semibold text-foreground">No results yet</p>
					<p class="max-w-md">Try adjusting your keywords or filters to discover more places.</p>
					<Button variant="outline" class="rounded-full" @click="performSearch">Search Again</Button>
				</div>
