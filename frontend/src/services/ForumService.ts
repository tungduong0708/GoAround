...
class ForumService {
    ...
    async getPosts(query?: IForumSearchQuery): Promise<IForumPost[]> {
        try {
            const response = await commonInstance.get('/forum/posts', { params: query });
    ...
    async replyToPost(postId: string, input: ICreateReplyInput): Promise<IForumReply> {
        try {
            const response = await authInstance.post(`/forum/posts/${postId}/replies`, input);
